...
import httpx
from schemas.auth import TokenData, RegisterOrganization, RegisterParticipant
...
from shared.models.user_technology_stack import UserTechnologyStack
...
async def get_user_by_email_from_user_service(email: str):
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(f"http://user_service:8000/users/by-email", params={"email": email})
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User:
    user_data = await get_user_by_email_from_user_service(email)
    if not user_data:
